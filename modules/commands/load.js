...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (command['onLoad']) try {
                ...
                throw new Error('『 '+ global.config.ICON +' 』 ➜  Không thể onLoad module, lỗi: ' + JSON.stringify(error), 'error');
            }
  ...
  if (errorList.length != 0) {
    ...
    api.sendMessage('『 '+ global.config.ICON +' 』 ➜  Đã tải thành công ' + (moduleList.length - errorList.length) + ' lệnh', threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    return api.sendMessage(`『 `+ global.config.ICON +` 』 ➜  Đã hủy thành công ${moduleList.length} lệnh`, threadID, messageID);
}
...
module.exports.run = function ({ event, args, api }) {
    ...
    switch (args[0]) {
      ...
      let commands = client.commands.values();
		  let infoCommand = "";
			api.sendMessage("『 "+ global.config.ICON +" 』 ➜ Hiện tại đang có " + client.commands.size + " lệnh có thể sử dụng!"+ infoCommand, event.threadID, event.messageID);
      break;
		}
        case "mdl": {
            if (moduleList.length == 0) return api.sendMessage("『 "+ global.config.ICON +" 』 ➜  Tên module không được để trống!", threadID, messageID);
        ...
        case "info": {
            ...
            if (!command) return api.sendMessage("『 "+ global.config.ICON +" 』 ➜ Module bạn nhập không tồn tại!", threadID, messageID);
